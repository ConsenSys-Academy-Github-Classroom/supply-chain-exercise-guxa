...
contract SupplyChain {
  ...
  struct Item {
	  ...
	  uint price;
	  State state;
	  address payable seller; //payable staviv prvo
  ...
  modifier forSale(uint _sku) {
	  bool isItemAdded = bytes(items[_sku].name).length != 0;
	  require(isItemAdded && items[_sku].state == State.ForSale, "Item is not for sale");
	  _;
  }
